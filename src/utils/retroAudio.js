...
// 8-bit Sword Slash Sound (Button B)
export function playSlashSFX() {
  if (isMuted) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
// 8-bit Death Sound (descending sweep)
...
// 8-bit Enemy Killed / Stomp Sound
export function playKillSFX() {
  ...
  try {
    ...
    const notes = [523.25, 784.00]; // C5, G5
    notes.forEach((freq, idx) => {
      const osc = ctx.createOscillator();
      const gain = ctx.createGain();
      ...
      gain.gain.setValueAtTime(0.16, ctx.currentTime + idx * 0.06 + 0.01);
...
// 8-bit Revive / Level Complete Fanfare
export function playReviveSFX() {
  if (isMuted) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    ...
    notes.forEach((freq, idx) => {
      ...
      gain.gain.setValueAtTime(0.18, ctx.currentTime + idx * 0.08 + 0.01);
